...
Before starting let us first have a top level understanding of the app creation process. The overall process can be divided in 3 parts- **UI**, **Notebook**, and **Variables**.
In **UI**, we drag and drop the UI elements to build our app design. This defines how our app will look and what features and elements will it have for the user. Next in **Notebook** we create requests to be made from the multiple app resources we have at our peril. 
And finally in **Variables**, we connect the **UI** (or the Design) with **Notebook** (or the data). Variables tab empowers us to enter an input (and hit a command) and the App will run through a relevant data repository and provide an output.
...
We use the **Notebook tab** and the **Variables tab** to connect the CFG platform resources like Model, Database, Storage, etc. with the App.
