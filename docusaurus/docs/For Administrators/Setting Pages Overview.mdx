...
![Data Apps](../../static/img/Settings/dataappshomepage.png)
...
![Update Project](../../static/img/Settings/updateproject.png)
...
Jobs section can be used to automate tasks which needs to be execute at periodic time intervals. To access, click on the Jobs button housed in the Settings Home page.

![Jobs Home Page](../../static/img/Settings/jobshomepage.png)

Here you can see the different jobs which are live currently. Let's use these jobs to understand the function of this section.

First, you need to give a 'Name' to the job. Then you can need to set the 'frequency'. Frequency is when the job will get executed everytime. In the image attached above, the first job's frequency mentioned is Daily at 12 pm, which means this job will get executed everyday at 12 noon.

Next is timezone which indicates the time zone of the time mentioned in frequency. Last run indicates the last time the job was executed. Status Active means currently live, Modified by indicated the ID of the account which last made edits to the job.

And finally there is Actions which has three icons- The first arrow will make the job run, the pencil one is to edit details of the job, and the wastebin icon will delete the job.

How to add a new job?

Click on Add button, you will see the below UI.

![Add Job](../../static/img/Settings/addjob.png)

You need to decribe your task in the pixel space. Fill up rest of the details and click on Add to create a new job. Your new job will be visible on the jobs home page.
