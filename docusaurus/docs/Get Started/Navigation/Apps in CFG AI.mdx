...
* To access the BI tool, login to the [CFG AI](https://workshop.cfg.deloitte.com/cfg-ai-demo/SemossWeb/packages/client/dist/#/) platform
...
### 3. Playground
This App is used to test various prompts with different AI models using chats. You can select type of model from drop down based on type of prompt that will be used and check if its working correctly. If a model dosn't support a particular prompt, it will provide that information in response. Its Front-end is a React App and backend is connected CFG.AI without any code. You can click on settings icon highlighted below to edit LLM parameters like temperature, token length etc.
...
To access all CFG.AI Apps or download them in your local environment, we can use below mentioned ways -
